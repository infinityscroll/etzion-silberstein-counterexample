...
static int code_class=2;
static constexpr std::array<unsigned,12> D2={32771,16398,8196,4104,2058,1029,524,260,140,72,47,22};
static constexpr std::array<unsigned,12> D3={32781,16396,8206,4106,2054,1029,516,268,140,70,41,20};
static unsigned matrix_of(unsigned x){unsigned m=0;const auto& b=code_class==2?D2:D3;for(int i=0;i<12;++i)if((x>>i)&1)m^=b[i];return m;}
static std::array<unsigned,4> rows_of(unsigned m){std::array<unsigned,4> r{};for(int i=0;i<4;++i)r[i]=(m>>(4*i))&15U;return r;}
...
int main(int argc,char**argv){
  if(argc<7){std::cerr<<"usage: class reps.jsonl lane lanes max_count out.jsonl [hit.json]\n";return 1;}
  code_class=std::stoi(argv[1]);if(code_class!=2&&code_class!=3)return 2;
  std::string repfile=argv[2],outfile=argv[6],hitfile=argc>7?argv[7]:"orbit_hit.json";int lane=std::stoi(argv[3]),lanes=std::stoi(argv[4]),maxcnt=std::stoi(argv[5]);
